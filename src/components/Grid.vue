...
function flipPointer($event: PointerEvent): void {
  ...
  if (el.dataset.row && el.dataset.column) {
...
<template>
  <div class="landscape:basis-[calc(100dvh-3rem)] portrait:max-h-[100dvw] portrait:grow flex justify-center items-center touch-none">
    <div class="aspect-square landscape:w-[stretch] landscape:max-w-[calc(100dvh-3rem)] portrait:h-[stretch] portrait:max-h-[100dvw]">
      <div class="px-1 grid grid-cols-[repeat(28,minmax(0,1fr))] grid-rows-[repeat(28,minmax(0,1fr))] landscape:px-2 landscape:pb-4">
        <template v-for="(row, rowIndex) in store.pixelDots" :key="rowIndex">
          <Dot
            v-for="(dot, columnIndex) in row"
            :key="columnIndex"
            :isWhite="!!dot"
            @pointerdown="touchDown"
            :row="rowIndex"
            :column="columnIndex"
            @flip-pointer="flipPointer"
            @flip="flip(rowIndex, columnIndex)"
            @pointerup="touchUp"
            @pointercancel="touchUp" />
        </template>
      </div>
    </div>
  </div>
